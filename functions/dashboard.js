...
function diffInMonthsAndDays(from, to) {
  ...
  if (fromDate.getDate() > toDate.getDate()) {
    days =
      daysInMonth[toDate.getMonth()] - fromDate.getDate() + toDate.getDate();
    months -= 1;
  } else {
...
exports.generation = async function func(guild) {
  ...
  const date = new Date();
  const time = date
    .toFormat("YYYY年 MM月DD日 HH24:MI:SS")
    ...
    if (now > testStart) {
      if (now > testEnd) {
        ...
        if (now > testEnd - 86400000) {
          ...
          test = `テスト${Math.floor(
            (now - testStart) / 86400000 + 1
          )}日目です(〜${data.nextTest[0][3]}月${data.nextTest[0][4]}日)`;
        }
      }
    } else {
