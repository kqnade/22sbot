...
async function run() {
  ...
  const data = await rest.get(
    Routes.applicationCommands(config.client, config.server)
  );
  console.log("---コマンド一覧---");
  ...
  switch (mode) {
    case "登録(更新)": {
      console.log("---追加コマンド---");
      //差分を確認
      for (const filterElement of commands.filter(
        (v) => !data.map((e) => e.name).includes(v.name)
      )) {
      ...
      if (prompt) {
        ...
        await rest
          ...
          .then((data) =>
            console.log(
              `${data.length} 個のアプリケーション コマンドが正常に登録されました。`
            )
          )
          .catch(console.error);
      }
    ...
    case "削除": {
      ...
      for (const selectedElement of selected) {
        ...
        await rest
          ...
          .then(() =>
            console.log(
              `アプリケーション コマンド"${selectedElement.name}"が正常に削除されました`
            )
          )
          .catch(console.error);
      }
