...
module.exports = [
  ...
  },
  {
    ...
    async execute(interaction) {
      if (
        interaction.options.data[5].value > 0 &&
        interaction.options.data[5].value < 5
      ) {
        ...
        await interaction.reply({
          ...
          }年${data.nextTest[interaction.options.data[5].value - 1][1]}月${
            data.nextTest[interaction.options.data[5].value - 1][2]
          }日〜${data.nextTest[interaction.options.data[5].value - 1][3]}月${
            data.nextTest[interaction.options.data[5].value - 1][4]
          }日に設定しました`,
          ephemeral: true,
        });
      } else {
        await interaction.reply({
          content: "どっか〜ん　するから、1~4の中で指定してくれ",
          ephemeral: true,
        });
      }
    },
  },
  {
    ...
    async execute(interaction) {
      ...
      await interaction.reply({
        content: `メッセージID:${data.dashboard[0]} を、自動更新ダッシュボードに設定しました。`,
        ephemeral: true,
      });
    },
  },
